resource "aws_autoscaling_group" "consul_gateway" {
  ...
  target_group_arns         = [aws_lb_target_group.consul_gateway.arn]
...
resource "aws_launch_template" "consul_gateway" {
  ...
  tag_specifications {
    ...
    tags = {
      Name = "consul-gatway-${var.datacenter}",
      role = "consul-gateway-${var.datacenter}",
    }
  }  
  update_default_version = true

  user_data = base64encode(templatefile("${path.module}/templates/consul_gateway.sh.tftpl", { 
